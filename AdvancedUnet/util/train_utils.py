...
from network.AdU import AdUNet
from util.image_utils import save_image
...
def init_nets(opt, net_path, device, tag=''):
    ...
    # cur_path = r"C:\Users\Breeze\Desktop\visual_motif_removal-master\checkpoints\RSU\net_baseline_RSU16.pth"
    if os.path.isfile(cur_path) and eval('net_baseline') is not None:
        print('loading baseline from %s/' % net_path)
        print('%s model loaded' % cur_path)
