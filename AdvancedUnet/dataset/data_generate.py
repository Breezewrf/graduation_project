...
from util.cache_utils import load_globals, init_folders
...
'''
    每一个dataset_tag可以对应一个cache_root，代表一个数据集
    比如说可以选择中文字符水印、彩色字符水印，对应的configuration也不同，
    在训练时也会加载不同的train_param，便于对比训练
'''
images_root = '../data/coco/'
cache_root = './data/%s' % dataset_tag
...
def save_np_image(images, folder, suffix, start_count=0):
    ...
    for image_index in range(images.shape[0]):
        image_path = '%s/%s/%d.%s' % (folder, suffix, image_index + start_count, save_extension)
